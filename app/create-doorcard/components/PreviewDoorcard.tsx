            ...
            {timeSlots.map((slot) => (
              <tr key={slot.value}>
                ...
                {days.map((day) => {
                  ...
                  const activeBlock = dayBlocks.find((block) => isTimeBlockAtSlot(block, slot.value))

                  if (activeBlock) {
                    const duration = Math.ceil(
                      (getTimeInMinutes(activeBlock.endTime) - getTimeInMinutes(activeBlock.startTime)) / 30,
                    )
                    ...
                    return (
                      <td
                        key={`${day}-${slot.value}`}
                        rowSpan={duration}
                        className={`${getActivityStyle(activeBlock.activity)} p-2 text-xs border-r last:border-r-0 border-b border-gray-200`}
                      >
                        <div>{formatTimeRange(activeBlock.startTime, activeBlock.endTime)}</div>
                        <div>{activeBlock.activity}</div>
                      </td>
                    )
                  }

                  const isInExistingBlock = dayBlocks.some((block) => isTimeInBlock(block, slot.value))
