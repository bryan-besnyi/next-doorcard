...
export default function PrintDoorcardPage() {
  ...
  useEffect(() => {
    const fetchDoorcard = async () => {
      ...
      console.log(`Fetching doorcard with ID: ${doorcardId}, isDraft: ${isDraft}`)

      try {
        const endpoint = isDraft ? `/api/doorcards/draft/${doorcardId}` : `/api/doorcards/${doorcardId}`
  ...
  console.log("Rendering PrintExportDoorcard component")
  return (
